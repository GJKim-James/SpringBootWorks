...
public class MelonMapper extends AbstractMongoDBComon implements IMelonMapper {
    ...
    public int insertSong(List<MelonDTO> pList, String colNm) throws Exception {
        ...
        // 데이터를 저장할 컬렉션 생성
        super.createCollection(colNm, "collectTime"); // 부모 추상활 클래스의 인덱스 생성하여 컬렉션 생성
        
        // 저장할 컬렉션 객체 생성
        MongoCollection<Document> col = mongodb.getCollection(colNm);
        
        for (MelonDTO pDTO : pList) { // 수집된 멜론 순위 노래 정보들을 반복문을 활용하여 1건씩 저장하기
            if (pDTO == null) {
                pDTO = new MelonDTO();
            }
    ...
    public List<MelonDTO> getSongList(String colNm) throws Exception {
        ...
        // 조회된 결과를 전달하기 위한 객체 생성하기
        List<MelonDTO> rList = new LinkedList<>();

        MongoCollection<Document> col = mongodb.getCollection(colNm);

        // 조회 결과 중 출력할 컬럼들(projection은 SQL의 SELECT절과 FROM절 가운데 들어가는 컬럼들이라고 생각하면됨)
        Document projection = new Document();
        projection.append("song", "$song");
        projection.append("singer", "$singer");

        // MongoDB는 무조건 ObjectID가 자동 생성되며, ObjectID는 사용하지 않는다면 조회할 필요가 없음
        // ObjectId를 가지고 오지 않을 때 사용함
        projection.append("_id", 0);

        // MongoDB의 find 명령어를 통해 조회할 경우 사용함
        // 조회하는 데이터의 양이 적은 경우, find를 사용하고, 데이터 양이 많은 경우는 무조건 Aggregate를 사용한다.
    ...
    public List<MelonDTO> getSingerSong(String pColNm, String pSinger) throws Exception {
        ...
        // MongoDB는 무조건 ObjectId가 자동 생성되며, ObjectId를 사용하지 않는다면 조회할 필요가 없음
        // ObjectId를 가지고 오지 않을 때 사용함
        projection.append("_id", 0);

        // MongoDB의 find 명령어를 통해 조회할 경우 사용함
        // 조회하는 데이터의 양이 적은 경우, find를 사용하고, 데이터 양이 많은 경우에는 무조건 Aggregate 사용한다.
    ...
    @Override
    public int updateSinger(String pColNm, String pSinger, String pUpdateSinger) throws Exception {
        ...
        // 조회할 조건(SQL의 WHERE 역할 / SELECT * FROM MELON_20220405 where singer = '방탄소년단')
        Document query = new Document();
        query.append("singer", pSinger);

        // MongoDB 데이터는 반드시 컬렉션을 조회하고, 조회된 ObjectId를 기반으로 데이터를 삭제함
    ...
    public int updateManySong(String pColNm, String pSinger, String pUpdateSinger, String pUpdateSong) throws Exception {
        ...
        // 한 줄로 append 해서 수정할 필드 추가해도 되지만, 가독성이 떨어져 줄마다 append 함
        ...
        // 위에 선언한 updateDoc 객체 데이터대로 수정하기
